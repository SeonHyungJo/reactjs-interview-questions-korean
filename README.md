...
247. ### displayName 클래스 속성의 목적은?
     ...
     displayName 문자열은 메시지 디버깅에 사용된다. 일반적으로 컴포넌트를 정의하는 함수 또는 클래스 이름에서 유추되므로 명시적으로 설정할 필요 없다. 디버깅 목적으로 다른 이름을 표시하거나 고차 컴포넌트를 작성할 때 명시적으로 설정할 수 있다.
     예를 들어, 디버깅을 쉽게 하려면 withSubscription HOC의 결과임을 알리는 표시 이름을 선택한다.
